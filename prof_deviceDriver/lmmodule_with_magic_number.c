...
#define  DEVICE_NAME "LM_module"    ///< The device will appear at /dev/LM_module using this value
...
static long int LM_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
	...
	int x;
	copy_from_user(&x, (int *)arg, sizeof(int));
	// copy_from_user (&x, (int *)arg, sizeof (int));

	switch(cmd){
		case SET:
			printk(KERN_INFO "LM: Executing SET. Received value: %d\n", x);
			break;
		case RESET:
	 		printk(KERN_INFO "LM: Executing RESET. Received value: %d\n", x);
	 		break;
	}
